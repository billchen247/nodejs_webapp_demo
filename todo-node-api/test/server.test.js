/* =============================================================================
 * test/server.test.js — integration tests for the Todo REST API
 * =============================================================================
 *
 ...
 * integration tests: routing, middleware, controllers, and file persistence
...
// Import from the new src/ layout. `server` is the http.Server exported by
...
const SEED_TODOS = [
    ...
    {
        id: 2,
        title: "Understand the HTTP module",
        completed: true,
...
test.before(async () => {
    ...
    // server on port 3000 and allows parallel test runs.
...
test("GET / serves the HTML home page", async () => {
    // The root URL used to 404. It now returns the landing page.
    const res = await fetch(`${baseUrl}/`);
    assert.strictEqual(res.status, 200);
    assert.match(res.headers.get("content-type"), /text\/html/);
    const body = await res.text();
    // Sanity-check the HTML actually looks like our template.
    assert.match(body, /<title>Todo Node\.js API<\/title>/);
...
test("GET /api/todos?completed=true filters completed todos", async () => {
    // Demonstrates QUERY PARAMETERS — the filter is not part of the path.
...
test("GET /api/todos/:id returns 400 when the id is not a positive integer", async () => {
    // "abc" isn't a number, so we expect 400 (malformed request) rather
    // than 404 (URL is fine, resource is missing).
...
test("POST /api/todos creates a new todo and returns 201 with the created record", async () => {
    ...
    // Server generates id, completed, createdAt for us.
    ...
    // Confirm it's really in the collection now.
...
test("POST /api/todos returns 400 when the request body is invalid JSON", async () => {
    // Non-JSON body must be caught by the body middleware and translated
    // into a clean 400 (not a 500 with a stack trace).
    const res = await fetch(`${baseUrl}/api/todos`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
...
test("PUT /api/todos/:id updates fields and returns the updated todo", async () => {
    const res = await fetch(`${baseUrl}/api/todos/1`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ title: "Learn Node.js HTTP", completed: true }),
    });

    assert.strictEqual(res.status, 200);
    const updated = await res.json();
    assert.strictEqual(updated.id, 1);
    assert.strictEqual(updated.title, "Learn Node.js HTTP");
...
test("DELETE /api/todos/:id removes the todo and returns 204 with no body", async () => {
    ...
    // 204 = success, no body. res.text() should therefore be "".
    assert.strictEqual(res.status, 204);
    const text = await res.text();
    assert.strictEqual(text, "");

    // Confirm the todo is really gone.
